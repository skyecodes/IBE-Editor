...
public final class ClientNetworkEmitter {
    ...
    public static void sendBlockEditorRequest(BlockPos blockPos, boolean nbt) {
        send(Networking.BLOCK_EDITOR_REQUEST, new BlockEditorRequestPacket(blockPos, nbt));
    }

    public static void sendEntityEditorRequest(int entityId, boolean nbt) {
        send(Networking.ENTITY_EDITOR_REQUEST, new EntityEditorRequestPacket(entityId, nbt));
    }
