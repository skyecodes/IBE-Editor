...
public class ItemEnchantmentsEditorCategoryModel extends AbstractItemEditorCategoryModel {
    ...
    protected void setupEntries() {
        ...
        Minecraft.getCommon().getRegistries().getEnchantments()
                .stream()
    ...
    public void apply(CompoundTag nbt) {
        newEnch = Minecraft.getCommon().getTagFactory().createListTag();
        super.apply(nbt);
        if (!newEnch.isEmpty()) {
    ...
    private void addNewEnchantment(String id, int lvl) {
        if (lvl != 0) {
            CompoundTag tag = Minecraft.getCommon().getTagFactory().createCompoundTag();
            tag.putString("id", id);
            tag.putInt("lvl", lvl);
            newEnch.addTag(tag);
        }
