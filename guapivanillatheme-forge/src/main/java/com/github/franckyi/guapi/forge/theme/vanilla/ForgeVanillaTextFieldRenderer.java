...
import com.github.franckyi.guapi.forge.theme.vanilla.mixin.ForgeTextFieldWidgetMixin;
...
public class ForgeVanillaTextFieldRenderer extends EditBox implements ForgeVanillaDelegateRenderer {
    private final TextField node;

    public ForgeVanillaTextFieldRenderer(TextField node) {
        super(Minecraft.getInstance().font, node.getX(), node.getY(), node.getWidth(), node.getHeight(), (Component) node.getLabel());
        this.node = node;
        ...
        node.labelProperty().addListener(newVal -> setMessage((Component) newVal));
    ...
    public Component renderText(String str, int firstCharacterIndex) {
        return node.getTextRenderer() == null ? new TextComponent(str) : (Component) node.getTextRenderer().render(str, firstCharacterIndex);
    }
    ...
    public void renderButton(@Nonnull PoseStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if (isVisible()) {
            ...
            if (flag1) {
                if (flag2) {
                    GuiComponent.fill(matrices, k1, i1 - 1, k1 + 1, i1 + 1 + 9, -3092272);
                } else {
