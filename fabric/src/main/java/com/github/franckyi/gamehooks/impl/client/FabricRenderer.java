...
public class FabricRenderer implements Renderer {
    ...
    public int getFontWidth(Text text) {
        return font().getWidth((net.minecraft.text.Text) text.getText());
    }

    @Override
    public void drawString(Matrices matrices, Text text, float x, float y, int color, boolean shadow) {
        if (shadow) {
            font().drawWithShadow(matrices.getMatrixStack(), (net.minecraft.text.Text) text.getText(), x, y, color);
        } else {
            font().draw(matrices.getMatrixStack(), (net.minecraft.text.Text) text.getText(), x, y, color);
        }
    }
    ...
    public void drawTooltip(Matrices matrices, Text text, int x, int y) {
        MinecraftClient.getInstance().currentScreen.renderTooltip(matrices.getMatrixStack(), (net.minecraft.text.Text) text.getText(), x, y);
    }
}
